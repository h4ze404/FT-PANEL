...
public class FishingManager {
    ...
    private static final double BITE_VELOCITY = -0.12; // рывок поплавка вниз (после устояния)
    private static final double BITE_DROP = 0.08;      // провал поплавка ниже точки поклёвки
    ...
    private volatile static boolean serverBiteFlag;
    ...
    private void tickCasting(MinecraftClient mc) {
        ...
        if (rodSlot < 0) {
            AFBaritoneClient.msg("Нет удочки в хотбаре! Стоп.");
            stop(false);
            return;
        }
