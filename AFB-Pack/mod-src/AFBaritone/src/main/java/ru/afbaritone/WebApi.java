...
public class WebApi {
            ...
            if (mc.currentScreen instanceof HandledScreen<?> hs) {
                ...
                for (Slot slot : hs.getScreenHandler().slots) {
                    ...
                    sb.append("{\"id\":").append(slot.id)
                      ...
                      .append(",\"ench\":").append(enchJson(st))
    ...
    /** Зачарования предмета: ["Острота V", "Прочность III", ...] */
    private static String enchJson(ItemStack stack) {
        StringBuilder sb = new StringBuilder("[");
        try {
            // Обычные чары (предметы) + зачарованные книги (stored)
            ...
            enchJsonAppend(sb, ench);
            enchJsonAppend(sb, stored);
        } catch (Throwable ignored) {
        }
        return sb.append(']').toString();
    }

    private static void enchJsonAppend(StringBuilder sb, net.minecraft.component.type.ItemEnchantmentsComponent ench) {
        if (ench == null || ench.isEmpty()) return;
        for (var e : ench.getEnchantmentEntries()) {
            if (sb.length() > 1) sb.append(',');
            String name = stripColor(net.minecraft.enchantment.Enchantment
                    .getName(e.getKey(), e.getIntValue()).getString());
